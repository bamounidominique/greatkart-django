...
def login(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            # While login, need to check if there is cartItem for that session,
            # If yes, assigned these items to user.
            try:
                ...
                if is_cart_item_exists:
                    ...
                    # Try to group the products by variations
                    product_variation = []
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.save()
            messages.success(request, "Password reset successfulll")
            return redirect('login')
        else:
